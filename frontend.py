...
def process_image():
    global INPUT_IMAGE_PATH, PROCESSED_IMAGE
    ...
    if file_path:
        ...
        if show_text.get():
            ...
            def process_RCNN():
                ...
                try:
                    processed_image = UseFasterRCNN.detect_and_label_image_Faster_rcnn(processed_image)
...
def update_text():
    if show_text.get():
        ...
        switch_label.config(text="Method of Image Processing: Faster R-CNN")
    else:
        # result_label.config(text="Processing Complete - Yolo-V5")
        switch_label.config(text="Method of Image Processing: Yolo-V5")
        
# Download the processed image, raise error if there is no image inputted or processed
